...
public class ServiceImpl implements IService {
    ...
    public List<Service> serviceList() {
        try {
            ...
            while (rs.next()){
                ...
                Service serviceS=new Service(sid,cid,title,info,days,date,price,status,customerS);
    ...
    public int serviceUpdate(Service service) {
        ...
        int status = 0;
        try {
            String sql =  "update Service set title = ?, info = ?, days = ?, price = ?, status=? where cid = ?";
            PreparedStatement pre = db.connect().prepareStatement(sql);
